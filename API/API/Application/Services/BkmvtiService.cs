...
namespace API.Application.Services
{
    public class BkmvtiService : IBkmvtiService
    {
        ...
        public async Task<ApiResponse<CustomerBilling>> GetAllCustomerBilling(
            DateTimeOffset dateDebutFacturation,
            string ncpf,
            DateTimeOffset? dateFinFacturation = null
        )
        {
            var result = await _bkmvtiRepository.GetAllCustomerBilling(
                dateDebutFacturation,
                ncpf,
                dateFinFacturation
            );

            return ApiResponse<CustomerBilling>.SuccessResponse(
                result,
